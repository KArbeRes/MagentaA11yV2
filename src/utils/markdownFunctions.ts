...
export const getMarkdownFunctionMap = (
  ...
  goToNext: () => {
    ...
    if (list) {
      ...
      let currentIndex = Array.from(list.children).findIndex(
  ...
  alertSuccess: () => {
    ...
    if (alertDiv) {
      alertDiv.innerHTML.length === 0
        ? (alertDiv.innerHTML = "Success: Your account has been updated!")
        : (alertDiv.innerHTML = "");
    }
  },
  ...
  alertWarning: () => {
    ...
    if (warningDiv) {
      warningDiv.innerHTML.length === 0
        ? (warningDiv.innerHTML = "Warning: The correct answer is all of them")
  ...
  increaseSelectStepper: () => {
    ...
    const stepperSelect = document.getElementById(
      "stepper"
  ...
  textareaKeyup: (event) => {
    ...
    const charsElement = document.getElementById("currentChars");

    if (charsElement) {
      charsElement.innerHTML = String(Number(maxLength) - currentLength);
    }
  },
});
